	...
	>
		<div class="flex flex-grow flex-col overflow-y-auto p-2.5">
			...
			<div class="mt-4 flex flex-col">
				<nav v-if="sidebar" class="flex-1 space-y-1 pb-4 text-base">
					<Tooltip
			...
			<div class="mt-auto flex flex-col items-center gap-2 text-base text-gray-600">
				<Button v-if="is_admin" variant="ghost" @click="open('https://docs.frappeinsights.com')">
				...
				<Dropdown
					...
					:options="[
						...
						// session.user.is_admin ?
							 {
									label: session.user.is_admin ? 'Switch to Desk' : 'Nhập dữ liệu',
									icon: 'grid',
									onClick: () => open('/app'),
							  },
							// : null,
...
const goBackFrappe = (href)=>{
	if(href){
		open('/app')
	}
}
const sidebar = computed(() => {
	return session.user.is_admin ? sidebarItems.value : userSidebarItems.value
})
